...
org_gtf_fasta <- data.frame(organism = c("yeast", "fruitfly", "zebrafish", "mouse", "human"),
                            ...
                            fasta = c("/Volumes/USELESS/DATA/fasta/saccharomyces_cerevisiae/pep/Saccharomyces_cerevisiae.R64-1-1.pep.all.fa.gz",
                                      "/Volumes/USELESS/DATA/fasta/drosophila_melanogaster/pep/Drosophila_melanogaster.BDGP6.pep.all.fa.gz",
                                      "/Volumes/USELESS/DATA/fasta/danio_rerio/GRCz10/pep/Danio_rerio.GRCz10.pep.all.fa.gz",
                                      "/Volumes/USELESS/DATA/fasta/mus_musculus/pep/Mus_musculus.GRCm38.pep.all.fa.gz",
                                      "/Volumes/USELESS/DATA/fasta/homo_sapiens/pep/Homo_sapiens.GRCh38.pep.all.fa.gz"))
...
plot_path <- "/Volumes/USELESS/STALLING/PLOTS/nascent_peptide"

for (i in 1:length(organisms)) {
  org <- organisms[i]
  load(c(file.path(consensus_path, paste0(org, ".Rsave")))) # consensus
  consensus$aa <- floor(consensus$ss / 3) + 1
  consensus <- consensus[consensus$aa > 30]
  
  fasta_pep <- read.fasta(as.character(org_gtf_fasta[org_gtf_fasta$organism == org,]$fasta))
  names(fasta_pep) <- sapply(getAnnot(fasta_pep), function(x){strsplit(substring(x, regexpr("transcript:", x) + 11), split = " ")[[1]][1]})
  if (org == "zebrafish") {
    names(fasta_pep) <- sapply(names(fasta_pep), function(x){substring(x, 1, 18)})
  }
  fasta_pep <- fasta_pep[names(fasta_pep) %in% unique(as.character(consensus$seqnames))]
  
  nascent_peptide <- t(mapply(function(x,y){fasta_pep[[x]][(y-30):y]}, as.character(consensus$seqnames), consensus$aa))
  ...
  # control1
  nascent_peptide_random1 <- t(mapply(function(x,y){fasta_pep[[x]][(y-30):y]}, as.character(consensus$seqnames), consensus$random1))
  ...
  # control2
  nascent_peptide_random2 <- t(mapply(function(x,y){fasta_pep[[x]][(y-30):y]}, as.character(consensus$seqnames), consensus$random2))
...
# save workspace
# save.image("/Volumes/USELESS/STALLING/sequence/nascent_peptide.RData")
load("/Volumes/USELESS/STALLING/sequence/nascent_peptide.RData")
...
# the rest!
save(REST, file = "/Volumes/USELESS/STALLING/structure/rest_not_explained_by_AA.Rsave")
